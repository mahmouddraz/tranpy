...
'''TO DO:, 
THIS NEEDS MAJOR REFACTORING AND A LOT OF CLEANING'''

index_feature_names = {}
bus = 1
for i in range(77):
    if i % 2 == 0:
        index_feature_names[i] = ('bus_' + str(bus) + ':u')
    else:
        index_feature_names[i] = ('bus_' + str(bus) + ':phi')
        bus += 1


def get_top_LIME_important_features(spobj, m):
    X = spobj.sp_explanations
    exps = []
    for explanation in X:
        explainer = explanation.local_exp.values()
        exps.append(explainer)
    ...
    result_lime_indices = list(records)
...
def get_top_SHAP_important_features(shap, m):
    feats = shap[1]
...
def get_top_important_features_LIME(spobj, m):
    indexes_lime = get_top_LIME_important_features(spobj, m)
...
def df_to_dataset(test_ds, shuffle, batch_size):
    pass
...
class SmartGridExplainer:
    ...
    def dnn_model_predict(self, x):
        ...
        test_ds = df_to_single_row(df, shuffle=False)
    ...
    # Generates a Local Lime Explanation explaining the instance i of the test set
    # Stores it in explainer outputs folder
    def generate_local_lime_explanation(self, i, path_localocal=None):
        explainer = lime_tabular.LimeTabularExplainer(training_data=np.array(self.X_train),
                                                      class_names=['unstable', 'stable'], mode="classification",
        ...
        if model_name == 'Sequential':
            if self.model.name == 'DNN':
                ...
                model_name = 'RNN'
            start = timer()
            exp = explainer.explain_instance(data_row=np.squeeze(self.X_test[i]), predict_fn=predict_fn,
                                             num_features=17)
            end = timer()
        else:
            start = timer()
            exp = explainer.explain_instance(data_row=np.array(self.X_test.iloc[i]),
                                             predict_fn=self.model.predict_proba, num_features=17)
            end = timer()
    ...
    def generate_global_lime_explanations(self):
        ...
        if my_file.is_file():
            ...
            start = timer()
            sp_obj = submodular_pick.SubmodularPick(explainer, np.array(self.X_train), predict_fn,
                                                    num_features=self.X_test.shape[1], num_exps_desired=5)
            end = timer()
    ...
    def path_global(self):
        ...
        path_global = os.path.join(dir_name, file_name)
    ...
    def generate_breakdown_explainer(self, i):
        ...
        smart_grid_exp = dx.Explainer(model, X_test, self.y_test,
                                      label=("Smart Grid New England " + model_name + " Pipeline on instance: " + str(
                                          i)))

        X_test.columns = self.feature_names
        instance = smart_grid_exp.predict_parts(instc,
                                                type='break_down')
    ...
    def generate_surrogate_explainer(self):
        ...
        logreg_predictions = logreg.predict(self.X_test)
        complex_model_predictions = self.model.predict(self.X_test)
        score = r2_score(logreg_predictions, complex_model_predictions)
        ...
        sort_features = list(map((lambda x: index_feature_names[x]), sort_index))
    ...
    # Get the indexes of the most important features

    def get_top_important_features_SHAP(self, shap, m):

        indexes_shap = get_top_SHAP_important_features(shap, m)
...
def df_to_single_row(df, shuffle=False, batch_size=32):
